...
export function useModalDragMove(context: UseModalDragMoveContext) {
  ...
  const drag = (wrap: any) => {
    ...
    const dialogTabsEl = wrap.querySelector('.ant-tabs-nav-wrap'); // 增加是Tba组件时，Tabs头部可拖拽
    const dialogHeaderEl = wrap.querySelector('.ant-modal-header');
    const dragDom = wrap.querySelector('.ant-modal');
    dialogTabsEl ? (dialogEl = dialogTabsEl) : (dialogEl = dialogHeaderEl);

    if (!dialogEl || !dragDom || !unref(context.draggable)) return;

    dialogEl.style.cursor = 'move';

    dialogEl.onmousedown = (e: any) => {
      if (!e) return;
      // 鼠标按下，计算当前元素距离可视区的距离
